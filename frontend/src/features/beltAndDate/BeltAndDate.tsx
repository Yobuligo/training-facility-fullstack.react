...
import styles from "./BeltAndDate.module.scss";
import { IBeltAndDateProps } from "./IBeltAndDateProps";

/**
 * This component is responsible for rendering a belt with its achievedAt date.
 */
export const BeltAndDate: React.FC<IBeltAndDateProps> = (props) => {
  ...
  const getStrokeClassName = (grade: Grade) => {
    switch (grade) {
      case Grade.KUP9:
        return componentStyles.kup9Outline;
      default:
        return componentStyles.kupOutline;
    }
  };
  ...
  const belt = (
    <svg
      ...
      className={style(styles.belt, gradeToClassName(props.grade))}
    >
      <path
      ...
      <path
        ...
        className={getStrokeClassName(props.grade)}
      />
      <path
        ...
        className={gradeToHalfBeltClassName(props.grade)}
      />
    </svg>
  );
  ...
  return (
    <div className={styles.beltAndDate}>
      ...
      <div className={style(styles.date)}>
        <div className={styles.year}>{DateTime.toYear(props.achievedAt)}</div>
        <div className={styles.monthAndDay}>
          <div>
            {DateTime.toDay(props.achievedAt)}
            {language === Language.DE && `.`}
          </div>
          <div>{renderMonth(DateTime.toMonth(props.achievedAt))}</div>
        </div>
      </div>
    </div>
  );
